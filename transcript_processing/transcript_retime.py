...
for ep1_transcript in df[df.episode_number == '735'].to_dict(orient='records'):
    ...
    transcript_texts.append(
        [x.text for x in parse_transcript(ep1_transcript).blocks])
...
# distance matrix (m, n)
D = np.zeros((len(transcript_texts[0]), len(transcript_texts[1])))
...
for i in range(len(transcript_texts[0])):
    for j in range(len(transcript_texts[1])):
        ...
        d = fuzz.partial_ratio(
            transcript_texts[0][i].lower(), transcript_texts[1][j].lower())
        if d <= 0:
