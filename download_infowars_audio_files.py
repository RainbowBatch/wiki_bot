...
def download_broadcast_details():
    ...
    for item in tqdm(soup.find_all("track")):
        ...
        ep_date = maya.when('%s-%s-%s' %
                            (ep_year, ep_month, ep_day), timezone='US/Eastern')
...
def download_broadcast_audio(year):
    ...
    relevant_episode_listing = episode_listing[
        episode_listing.release_date.str.contains(year, case=False)
        & episode_listing.show.str.contains('Alex', case=False)
    ]
...
if __name__ == '__main__':
    ...
    download_broadcast_audio(year='2012')
